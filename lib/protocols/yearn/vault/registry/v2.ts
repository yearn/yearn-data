...
export async function fetchAddressesExperimental(ctx: Context): Promise<string[]> {
  const registry = RegistryV2Contract__factory.connect(Registry, ctx.provider);
  ...
  return test.map((event) => event.args && event.args.vault).filter((address) => !prod.includes(address));
}
