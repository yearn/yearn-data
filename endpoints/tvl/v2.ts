...
function filter(vault: CachedVault) {
  return vault.type === "v2" && vault.endorsed && vault.tvl && !vault.emergencyShutdown;
}
...
export const handler = wrap(async () => {
  ...
  params.append("contract_addresses", vaults.map((vault) => vault.token.address).join(","));
  ...
  vaults = vaults.filter((vault) => !!prices[vault.token.address.toLowerCase()]);
  console.log(vaults);

  const tvl = vaults.reduce(
    (value: number, vault) => value + new BigNumber(vault.tvl ? vault.tvl.value : 0).toNumber(),
    0
  );
