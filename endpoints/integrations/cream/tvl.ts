...
export const handler = wrap(async () => {
  ...
  const pricesForTVL = prices.filter(
    (prices) =>
      prices.token_address != "0x7589C9E17BCFcE1Ccaa1f921196FDa177F0207Fc"
  ); // removing the token cy3CRV as this is already taken into account in TVL V1 YEARN.

  const total: creamAssets = pricesForTVL.reduce(
    (r: number, prices) =>
      r +
      parseFloat(prices.cash.value) * parseFloat(prices.underlying_price.value),
    0
  );
